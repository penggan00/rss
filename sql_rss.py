...
async def save_status(status):
    ...
    try:
        ...
        # 批量upsert
        response = supabase.table('rss_status').upsert(records).execute()
        logger.info(f"更新了 {len(records)} 条记录到数据库")
        
    except Exception as e:
        logger.error(f"保存数据库状态失败: {e}")
        ...
        try:
            ...
            for idx, url in enumerate(RSS_FEEDS):
                if message := await process_feed(session, url, status, bot):
                    ...
                    await save_status(status)  # 每个源处理后立即保存
            ...
            await save_status(status)  # 最终确保保存
