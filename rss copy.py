...
class RSSDatabase:
    ...
    async def save_status(self, feed_group, feed_url, entry_url, timestamp):
        if USE_PG:
            async with self.pg_pool.acquire() as conn:
                await conn.execute(
                    "INSERT INTO rss_status VALUES($1, $2, $3, $4) ON CONFLICT DO NOTHING",
                    feed_group, feed_url, entry_url, timestamp
                )
        else:
            def _save():
                c = self.conn.cursor()
                c.execute("INSERT OR IGNORE INTO rss_status VALUES (?, ?, ?, ?)",
                          (feed_group, feed_url, entry_url, timestamp))
                self.conn.commit()
                    ...
                    if feed_message:
                        try:
                            ...
                            for entry_id in pending_entry_ids:
                                await db.save_status(group_key, canonical_url, entry_id, time.time())
