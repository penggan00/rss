...
5 Remove all "|" symbols from the entire text
"""
        try:
        ...
        with imaplib.IMAP4_SSL(IMAP_SERVER) as mail:
            ...
            for num in nums[0].split():
                try:
                    ...
                    safe_message = escape(f"{header}{body}")  # 转义后的
                    raw_message = f"{header}{body}"           # 原始未转义的

                    chunks = MessageFormatter.split_content(safe_message, MAX_MESSAGE_LENGTH)
                    raw_chunks = MessageFormatter.split_content(raw_message, MAX_MESSAGE_LENGTH)  # 未转义的分块

                    for safe_chunk, raw_chunk in zip(chunks, raw_chunks):
                        await bot.send_message(raw_text=raw_chunk, escaped_text=safe_chunk, parse_mode='MarkdownV2')
