...
class EmailToTelegramBot:
            ...
            try:
               ...
               #     logging.info("主题安全翻译完成")
                ...
                if translated_content and translated_content != content:
                    ...
                    def stash_url(match):
                        url = match.group(0)
                        placeholder = f'\x00URL{len(url_store)}\x00'
                        url_store.append(url)
                        return placeholder
                    
                    protected = re.sub(r'https?://[^\s\)\]\}]+', stash_url, translated_content)
                    protected = protected.replace('_', 'ˍ')
                    
                    for i, url in enumerate(url_store):
                        protected = protected.replace(f'\x00URL{i}\x00', url)
    ...
    def clean_special_characters(self, text):
        """内容清理特殊字符：| 和 ---，确保保留换行符（URL 全程保护）"""
        if not text:
            return ""
        
        # ★★★ 第一步：把所有 URL 抠出来，用占位符替换 ★★★
        url_store = []
        ...
        # 匹配 http/https URL（直到遇到空格、右括号、右方括号、右花括号）
        text = re.sub(r'https?://[^\s\)\]\}]+', stash_url, text)
        
        # ★★★ 第二步：正常清理（此时 URL 已被藏起来，不会被误伤）★★★
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if translated and translated != text:
                    # 清理 ASS 样式串
                    if '\\fn' in translated and '\\fn' not in text:
                        print("⚠️ 翻译产生了 ASS 样式串！")
                        print(f"   原文: {repr(text[:200])}")
                        print(f"   译文: {repr(translated[:200])}")
    ...
    def translate_with_deepl(self, text):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logging.warning(f"⚠️ DeepL 返回状态码: {response.status_code}")
        except requests.exceptions.Timeout:
